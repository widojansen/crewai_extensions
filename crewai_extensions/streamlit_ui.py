...
class CrewAIStreamlitUI:
    ...
    def _display_config_page(self):
        ...
        """
        try:
            # Import the config module from the pages directory
            from .pages import config

            # Initialize the session state (if needed)
            self._initialize_session_state()

            # Call the run function, passing the current instance
            config.run(self)
        except ImportError as e:
            st.error(f"Error loading configuration page: {str(e)}")
            st.info("Make sure the config.py file exists in the pages directory.")
        except Exception as e:
            st.error(f"Error displaying configuration page: {str(e)}")
